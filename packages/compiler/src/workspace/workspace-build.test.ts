...
describe('workspace-package build', () => {
	...
	it('prepares one shared font asset while rebasing runtime and review URLs independently', async () => {
		const manifest = hostManifest({ files: ['generated/runtime', 'generated/assets'] });
		...
		expect(runtimeCss).toContain(`/cdn/tfs-fonts/${faceFile}`);
		expect(reviewFontCss).toContain(`./${faceFile}`);
		...
		expect(moduleFonts).toContain(`https://cdn.example/fonts/${faceFile}`);
		...
		expect(relativeCss).toContain(`../../assets/fonts/${faceFile}`);

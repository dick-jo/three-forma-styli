...
function assertExactExport(
	...
	if (!actual || typeof actual !== 'object' || Array.isArray(actual)) {
		throw new Error(
			`${manifestPath} export ${required.subpath} must provide exact ordered conditions ${display(required.target)}.`
		);
	}
	const conditions = actual as Record<string, unknown>;
	const conditionKeys = Object.keys(conditions);
	const requiredKeys = Object.keys(required.target);
	if (
		conditionKeys.length !== requiredKeys.length ||
		conditionKeys.some((condition, index) => condition !== requiredKeys[index])
	) {
		throw new Error(
			`${manifestPath} export ${required.subpath} conditions must be exactly ${display(requiredKeys)} in that order; received ${display(conditionKeys)}.`
		);
	}
	for (const condition of requiredKeys) {
...
function assertNoStaleGeneratedExports(
	exports: Record<string, unknown>,
	required: readonly RequiredPackageExport[],
	generatedFromHost: string,
	manifestPath: string
): void {
	const requiredSubpaths = new Set(required.map((entry) => entry.subpath));
	const generatedRoot = normalizedPackagePath(generatedFromHost);
	for (const [subpath, target] of Object.entries(exports)) {
		if (requiredSubpaths.has(subpath)) continue;
		const staleTarget = exportTargets(target).find((entry) => {
			const normalized = normalizedPackagePath(entry);
			return normalized === generatedRoot || normalized.startsWith(`${generatedRoot}/`);
		});
		if (staleTarget) {
			throw new Error(
				`${manifestPath} export ${subpath} points into TFS-owned output at ${display(staleTarget)}, but that export is not generated by the current project plan. Remove the stale export or re-enable its target.`
			);
		}
	}
}

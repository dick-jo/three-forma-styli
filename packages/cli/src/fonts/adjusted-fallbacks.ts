...
export async function buildAdjustedFallbacks(
	...
	const { fonts: sourceFonts, roles } = typography;
	if (!sourceFonts || !roles) return undefined;
	...
	try {
		...
		const fonts = Object.fromEntries(
			Object.entries(sourceFonts).map(([id, font]) => [
				id,
				usedFonts.has(id)
					? { ...font, fallbacks: [privateFamily(id), ...(font.fallbacks ?? [])] }
					: font,
			])
		) as NonNullable<TypographySystem['fonts']>;
		const css = [

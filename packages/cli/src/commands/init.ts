...
export async function initCommand(projectName?: string, options: InitOptions = {}): Promise<void> {
	try {
		...
		if (!(await fs.pathExists(themePath))) {
			...
			console.error(
				chalk.yellow(`  Available themes: ${availableThemes.map((t) => t.name).join(', ')}`)
			);
			process.exit(1);
		}
		...
		const themeSourceFiles = themeFiles.filter(
		...
		console.log(chalk.white(`  npx tfs build .`));
		console.log(

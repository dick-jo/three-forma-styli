...
export function getHeaderLines(info: FileHeaderInfo): string[] {
	const lines = ['Do not edit directly', `Generated by ${info.toolName} v${info.toolVersion}`];
 ...
 */
export function formatHeaderComment(lines: string[], style: CommentStyle = 'block'): string {
	switch (style) {
		case 'block':
			// /* */ style (CSS, JS, TS)
			return `/**\n${lines.map((l) => ` * ${l}`).join('\n')}\n */\n\n`;
